...
static const char *const kAsanReportAddressTemplate = "__asan_report_address_";
...
private:
  ...
  FunctionCallee AsanPrintAddressDescription[2][2][kNumberOfAccessSizes];
...
Value *AddressSanitizer::isInterestingMemoryAccess(Instruction *I,
  ...
  if (PtrOperand) {
    ...
    // with them.
    typeOfElement=cast<PointerType>(PtrOperand->getType());
...
void AddressSanitizer::instrumentMop(ObjectSizeOffsetVisitor &ObjSizeVis,
  ...
  SmallVector<char, 8> Vec;
  llvm::raw_svector_ostream RSO { Vec };
  typeOfElement->print(RSO, true, true);
  strPointer = IRB.CreateGlobalStringPtr(RSO.str());
  typeOfElement->dump();
  ...
  outs() << randomNumber;
  Value *randomNumberLong =  ConstantInt::get(IRB.getInt32Ty(), randomNumber);
  if (Exp == 0){
    if(IsWrite){
      Call =
        IRB.CreateCall(AsanPrintAddressDescription[IsWrite][0][AccessSizeIndex], { AddrLong, randomNumberLong, strPointer});
    }
  }
    else{
      if(IsWrite){
      Call = IRB.CreateCall(AsanPrintAddressDescription[IsWrite][1][AccessSizeIndex],
                            {AddrLong, randomNumberLong,ExpVal, strPointer});} }
...
void AddressSanitizer::instrumentAddress(Instruction *OrigIns,
  ...
  Value *ShadowValueLong = IRB.CreatePointerCast(ShadowPtr, IntptrTy);
  
  Value *Cmp = IRB.CreateICmpNE(ShadowValue, CmpVal);
Value *ExpVal = Exp == 0 ? nullptr : ConstantInt::get(IRB.getInt32Ty(), Exp);
Value *TypeSizeVal=ConstantInt::get(IRB.getInt32Ty(), TypeSize);
...
void AddressSanitizer::initializeCallbacks(Module &M) {
  IRBuilder<> IRB(*C);
  SmallVector<Type *, 2> Addr1{IntptrTy, IntptrTy};
  ...
  for (int Exp = 0; Exp < 2; Exp++) {
    for (size_t AccessIsWrite = 0; AccessIsWrite <= 1; AccessIsWrite++) {
      ...
      Type *randType = Type::getInt32Ty(*C);
      Type *ElementPtrTy = IRB.getInt8PtrTy();
      SmallVector<Type *, 3> Args3={IntptrTy,randType, ElementPtrTy};
      if (Exp) {
 ...
 AsanPrintAddressDescription[AccessIsWrite][Exp][AccessSizeIndex] = M.getOrInsertFunction(kAsanReportAddressTemplate + ExpStr + Suffix + EndingStr, FunctionType::get(IRB.getVoidTy(), Args3, false));
