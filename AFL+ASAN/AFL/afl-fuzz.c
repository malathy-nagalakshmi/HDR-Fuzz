...
int queue_index = 0;
int shmid_driver_to_afl;
int least_headrooms[6401];  //maintains least headroom values for every buffer access
int not_interesting_counter = 1;
int sync_flag = 0;

/*queue initilisation*/
int ch, k, x;
int f, r;
int size_internal = 6400;
f = -1;
r = -1;

struct queue_afl_to_driver
{
  char testcase[164];
  int interesting_afl;
  int softDelete;
};
struct queue_afl_to_driver *queue_afl_to_driver_to_driver;
...
struct queue_driver_to_afl *queue_driver_to_afl;
...
EXP_ST u64 total_crashes, /* Total number of crashes          */
    queue_cycle_10m,      /* Number of cycles since 15 mins   */
...
struct queue_entry
{
  ...
  int asan_favoured;  /*favoured by asan */
  int headroomChange; /*has least headroom changed */
  int position1;
  int bucketNumber;
  int bufferAccess;
  int asan_processed;
  ...
  u64 exec_us,               /* Execution time (us)              */
      last_path_found_cycle, /* Cycle in which a new path found  */
  ...
  struct queue_entry *next, /* Next element, if any             */
      ...
      *next_cov,            /* next element in cov              */
      *next_BO;             /* next element in BO goal          */
};
...
static struct queue_entry *queue, /* Fuzzing queue (linked list)      */
    ...
    *q_cov,                       /* Coverage linked list             */
    *q_BO,                        /* BO linked list                   */
    *q_cur_cov,                   /* Current offset within the cov q  */
    *q_cur_BO,                    /* Current offset within the BO q   */
    *q_top_cov,                   /* Top of the cov list              */
    *q_top_BO;                    /* Top of the BO list               */
...
static void cull_queue(void)
{
  ...
  while (q)
  {
    ...
    {
      if (strcmp((queue_driver_to_afl + queue_index)->testcase_name, "") != 0)
      {
        if (strcmp((queue_driver_to_afl + queue_index)->testcase_name, q->fname) == 0)
        {
          if (!q->asan_processed)
          {
            if ((queue_driver_to_afl + queue_index)->retained)
            {
              pending_favored_ab++;
              if (q_top_BO)
              {
          ...
          (queue_driver_to_afl + queue_index)->softDelete = 1;
          if ((queue_driver_to_afl + queue_index)->isCrash == 1)
          {
            if (q->favored = 1)
            {
              q->favored = 0;
          ...
          if (((queue_driver_to_afl + queue_index)->retained))
          {
            q->asan_favoured = (queue_driver_to_afl + queue_index)->retained;
            q->position1 = (queue_driver_to_afl + queue_index)->position;
            q->bufferAccess = (queue_driver_to_afl + queue_index)->bufferAccesses;
            q->headroomChange = (queue_driver_to_afl + queue_index)->leastHeadroom;
            if (least_headrooms[(queue_driver_to_afl + queue_index)->position] > (queue_driver_to_afl + queue_index)->leastHeadroom)
            {
              least_headrooms[(queue_driver_to_afl + queue_index)->position] = (queue_driver_to_afl + queue_index)->leastHeadroom;
            }
            q->favored = 1;
            if (!q->was_fuzzed)
              pending_favored++; 
          }
          queue_index = (queue_index + 1) % size_internal;
          break;
        }
        else
        {
...
int insert(u8 *x, struct queue_afl_to_driver *q, int *f, int *r, int size, int interesting)
{
  ...
  {
    *r = (*r + 1) % size;
    strcpy((q + *r)->testcase, x);
    (q + *r)->interesting_afl = interesting;
    (q + *r)->softDelete = 0;
    if (*f == -1)
      *f = 0;
    return 1;
...
static u8 save_if_interesting(char **argv, void *mem, u32 len, u8 fault)
{
  ...
  if (fault == crash_mode)
  {
    ...
    if (!sync_flag)
    {
      ...
      {
        //Sampling, save to sample_folder
        if (UR(100) < 10)
        {
          fn1 = alloc_printf("%s/queue/id:%06u,%s", "/home/temp/AFL/sample_dir", not_interesting_counter,
                             describe_op(hnb));
          fd1 = open(fn1, O_WRONLY | O_CREAT | O_EXCL, 0600);
          if (fd1 < 0)
            PFATAL("Unable to create '%s'", fn1);
          ck_write(fd1, mem, len, fn1);
          close(fd1);

          ck_free(fn1);

          not_interesting_counter = not_interesting_counter + 1;
        } 
        if (crash_mode)
          total_crashes++;
        return 0;
    ...
    queue_cur->last_path_found_cycle = queue_cycle;
    if (hnb == 2)
    {
    ...
    insert(fn, queue_afl_to_driver_to_driver, &f, &r, size_internal, 1);
  }
...
static u32 factor_for_ab(struct queue_entry *q, u32 iscore)
{
  u32 abscore, retFac = 1;
  u32 icVal, iHrg;
  abscore = iscore;
  /* set energy to zero for a non favored test BAP test case   */
  /* if is more than 10 mins and it has not discoved a new path */
  /* for more than 10 cycles                                    */
  if (!q->favored)
  {
    if (q->asan_favoured && queue_cycle_10m > 0 && (queue_cycle - q->last_path_found_cycle) > 5 && UR(2) < 1)
      return 0;
    else
    {
      abscore = abscore * 5;
  ...
  /* Only favorate test cases are considered */
  if (q->asan_favoured == 1)
  { /* is this ab favored ? */
    if (queue_cycle_10m > 0 && (queue_cycle - q->last_path_found_cycle) > 5 && UR(2) < 1)
      return 0;
    else
    {
      if (abscore < 100)
        abscore = 100; /* resetting the effect of execution time penalty */

      retFac = 1;
      /* Scoring based on minimum headroom */
      if (q->headroomChange < 4)
        retFac = 50; /* Previous value : 5 */
      else if (q->headroomChange < 32)
        retFac = 25; /* Previous value : 3 */
      else if (q->headroomChange < 64)
        retFac = 5; /* Previous value : 2 */
      abscore *= retFac;
    }
...
static u32 calculate_score(struct queue_entry *q)
{
  ...
  if (q->asan_favoured)
  { /* if it is not a asan-marked test case, skip it */

    perf_score = factor_for_ab(q, perf_score);
  }
...
static u8 fuzz_one(char **argv)
{
  ...
  if (pending_favored)
  {
    if (pending_favored_ab && prop_goal)
    {
      if ((queue_cur->was_fuzzed || !queue_cur->favored) &&
          queue_cur->asan_favoured == 0 && queue_cur->asan_processed &&
          UR(100) < SKIP_TO_NEW_PROB)
        return 2;
    }
    else if (prop_goal == 0 && queue_cur->asan_favoured == 0 && queue_cur->asan_processed)
    {
  ...
  if (prop_goal && queue_cur->asan_favoured == 0 && queue_cur->asan_processed)
    return 2;
#endif /* ^IGNORE_FINDS */
...
{
  int pos;
  for (pos = 0; pos < 6400; pos++)
    least_headrooms[pos] = 255;
  queue_afl_to_driver_to_driver = malloc(6400 * sizeof(struct queue_afl_to_driver));
  queue_driver_to_afl = malloc(6400 * sizeof(struct queue_driver_to_afl));
  key_t key_afl_to_driver = ftok("/home/temp/IISC", 65);
  s32 shmid_afl_to_driver = shmget(key_afl_to_driver, 1100800, 0666 | IPC_CREAT);
  queue_afl_to_driver_to_driver = shmat(shmid_afl_to_driver, 0, 0);
  key_t key_driver_to_afl = ftok("/home/temp/IISC/llvm/projects", 65);
  shmid_driver_to_afl = shmget(key_driver_to_afl, 1228800, 0666);
  queue_driver_to_afl = shmat(shmid_driver_to_afl, (void *)0, 0);
  ...
  /* Build initial cov,BO queue links for queue */
  q = queue;
  while (q)
  {
    if ((q->asan_favoured == 0 && q->asan_processed) || !q_top_cov)
    {
      if (q_top_cov)
      {
        q_top_cov->next_cov = q;
    ...
    if (q->asan_favoured == 1 || q->asan_favoured == 2)
    {
      if (q_top_BO)
      {
        q_top_BO->next_BO = q;
  ...
  while (1)
  {
    ...
    if (prop_goal)
    {
      ...
      {
        queue_cur = q_cur_BO;
        q_cur_BO = q_cur_BO->next_BO;
      }
    }
    if (!prop_goal || skip_bo_goal)
    {
      if (!q_cur_cov)
        q_cur_cov = q_cov;
      queue_cur = q_cur_cov;
      q_cur_cov = q_cur_cov->next_cov;
    }
    ...
    /* restore queue_cur */
    queue_cur = q_temp;
    /* prop_goal is used fuzz_one */
    /* To be fair, wait till each one is selected */
    if (skipped_fuzz == 0)
    {
      if (prop_goal)
        prop_goal = 0;
